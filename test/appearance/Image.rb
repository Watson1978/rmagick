...
class AppearanceMontageUT < Test::Unit::TestCase
  ...
  #   assert_same_image('expected/image_adaptive_blur_channel.jpg', new_image, delta: DELTA)
  # end

  def test_adaptive_sharpen_channel
    new_image = @image.adaptive_sharpen_channel(20, 5, Magick::RedChannel, Magick::BlueChannel)
    assert_same_image('expected/image_adaptive_sharpen_channel.jpg', new_image, delta: DELTA)
  end
  ...
  def test_auto_gamma_channel
    ...
    assert_same_image('expected/image_auto_gamma_channel.jpg', new_image, delta: DELTA)
  end

  def test_auto_level_channel
    new_image = @image.auto_level_channel(Magick::RedChannel, Magick::BlueChannel)
    assert_same_image('expected/image_auto_level_channel.jpg', new_image, delta: DELTA)
  end
  ...
  def test_bilevel_channel
    ...
    assert_same_image('expected/image_bilevel_channel.jpg', new_image)
  end

  def test_blend
    source = Magick::Image.read(image_path('images/compose_mask_source.gif')).first
    new_image = @image.blend(source, '90%', '20%', 50, 20)
    assert_same_image('expected/image_blend.jpg', new_image, delta: DELTA)
  end

  def test_displace
    source = Magick::Image.read(image_path('images/compose_mask_source.gif')).first
    new_image = @image.displace(source, 25, 25, Magick::CenterGravity, 10, 10)
    assert_same_image('expected/image_displace.jpg', new_image, delta: DELTA)
  end

  def test_blur_channel
    new_image = @image.blur_channel(10, 5, Magick::RedChannel, Magick::BlueChannel)
    assert_same_image('expected/image_blur_channel.jpg', new_image, delta: DELTA)
  end

  def test_border
    new_image = @image.border(20, 20, 'red')
    assert_same_image('expected/image_border.jpg', new_image, delta: DELTA)
  end

  def test_channel
    new_image = @image.channel(Magick::BlueChannel)
    assert_same_image('expected/image_channel.jpg', new_image, delta: DELTA)
  end
  ...
  def test_clut_channel
    ...
    assert_same_image('expected/image_clut_channel.jpg', @image, delta: DELTA)
  end
  ...
  def test_color_flood_fill
    ...
    assert_same_image('expected/image_color_flood_fill.jpg', new_image, delta: DELTA)
  end

  def test_colorize
    pixel = Magick::Pixel.new(Magick::QuantumRange)
    new_image = @image.colorize(0.4, 0.5, 0.6, 0.7, pixel)
    assert_same_image('expected/image_colorize.jpg', new_image, delta: DELTA)
  end
  ...
  def test_composite_tiled
    ...
    assert_same_image('expected/image_composite_tiled.jpg', new_image, delta: DELTA)
  end
  ...
  def test_constitute
    ...
    assert_same_image('expected/image_constitute.jpg', new_image, delta: DELTA)
  end

  def test_contrast
    new_image = @image.contrast(true)
    assert_same_image('expected/image_contrast.jpg', new_image, delta: DELTA)
  end

  def test_contrast_stretch_channel
    new_image = @image.contrast_stretch_channel('60%', '20%', Magick::RedChannel, Magick::GreenChannel)
    assert_same_image('expected/image_contrast_stretch_channel.jpg', new_image)
  end
  ...
  def test_morphology_channel
    ...
    assert_same_image('expected/image_morphology_channel.jpg', new_image)
  end

  def test_convolve
    kernel = [0.5, 1.0, 0.5, 1.0, 0.5, 1.0, 0.5, 1.0, 0.5]
    new_image = @image.convolve(1, kernel)
    assert_same_image('expected/image_convolve.jpg', new_image, delta: DELTA)
  end

  def test_convolve_channel
    kernel = [0.5, 1.0, 0.5, 1.0, 0.5, 1.0, 0.5, 1.0, 0.5]
    new_image = @image.convolve_channel(1, kernel, Magick::RedChannel, Magick:: BlueChannel)
    assert_same_image('expected/image_convolve_channel.jpg', new_image, delta: DELTA)
  end

  # def test_cycle_colormap
  #   new_image = @image.cycle_colormap(100)
  #   assert_same_image('expected/image_cycle_colormap.jpg', new_image)
  # end

  def test_equalize
    new_image = @image.equalize
    assert_same_image('expected/image_equalize.jpg', new_image)
  end

  def test_equalize_channel
    new_image = @image.equalize_channel(Magick::RedChannel, Magick::BlueChannel)
    assert_same_image('expected/image_equalize_channel.jpg', new_image, delta: DELTA)
  end

  def test_erase!
    @image.erase!
    assert_same_image('expected/image_erase_bang.jpg', @image, delta: DELTA)
  end

  def test_frame
    new_image = @image.frame(20, 20, 20, 20, 5, 5, 'red')
    assert_same_image('expected/image_frame.jpg', new_image, delta: DELTA)
  end
  ...
  def test_levelize_channel
    ...
    assert_same_image('expected/image_levelize_channel.jpg', new_image, delta: DELTA)
  end

  def test_linear_stretch
    new_image = @image.linear_stretch(Magick::QuantumRange / 2, Magick::QuantumRange)
    assert_same_image('expected/image_linear_stretch.jpg', new_image, delta: DELTA)
  end

  # def test_matte_flood_fill
  #   target_color = @image.pixel_color(100, 100)
  #   new_image = @image.matte_flood_fill(target_color, 100, 100, Magick::FillToBorderMethod, alpha: 0.5)
  #   assert_same_image('expected/image_matte_flood_fill.jpg', new_image, delta: DELTA)
  # end

  def test_modulate
    new_image = @image.modulate(0.5, 0.5, 0.5)
    assert_same_image('expected/image_modulate.jpg', new_image, delta: DELTA)
  end

  def test_negate
    new_image = @image.negate
    assert_same_image('expected/image_negate.jpg', new_image, delta: DELTA)
  end

  def test_negate_channel
    new_image = @image.negate_channel(false, Magick::RedChannel, Magick::BlueChannel)
    assert_same_image('expected/image_negate_channel.jpg', new_image, delta: DELTA)
  end

  def test_normalize
    new_image = @image.normalize
    assert_same_image('expected/image_normalize.jpg', new_image, delta: DELTA)
  end

  def test_normalize_channel
    new_image = @image.normalize_channel(Magick::RedChannel, Magick::BlueChannel)
    assert_same_image('expected/image_normalize_channel.jpg', new_image, delta: DELTA)
  end
