...
Image_extent(int argc, VALUE *argv, VALUE self)
{
    ...
    if (height <= 0L || width <= 0L)
    {
        ...
        {
            rb_raise(rb_eArgError, "invalid extent geometry %ldx%ld+%"PRIuSIZE"+%"PRIuSIZE""
...
build_inspect_string(Image *image, char *buffer, size_t len)
{
    ...
    {
        x += snprintf(buffer+x, len-x, "%"PRIuSIZE"x%"PRIuSIZE"+%"PRIuSIZE"+%"PRIuSIZE" ", image->page.width, image->page.height
    ...
    else
    {
        ...
        {
            x += snprintf(buffer+x, len-x, "PseudoClass %"PRIuSIZE"=>%ldc ", image->total_colors
                         , (long)image->colors);
